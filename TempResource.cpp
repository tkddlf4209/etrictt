...
TempResource::TempResource(std::string resourceUri):
        ...
        m_var_value_rt{}
{
    std::cout << "Running: TempResource constructor" << std::endl;
...
TempResource::~TempResource(void) { }
...
OCStackResult TempResource::registerResource(uint8_t resourceProperty)
{
    ...
    std::cout << "TempResource:" << std::endl;
...
double TempResource::getTemp(void)
{
    return m_var_value_temp;
}

void TempResource::setTemp(double temp)
{
  
        m_var_value_temp = temp;
        std::cout << "\t\t" << "property 'temperature': " << m_var_value_temp << std::endl;
    
}

OCStackResult TempResource::sendNotification(void)
{
...
OC::OCRepresentation TempResource::get(OC::QueryParamsMap queries)
{
    OC_UNUSED(queries);

    m_rep.setValue(m_var_name_temp, m_var_value_temp );
...
OCEntityHandlerResult TempResource::post(OC::QueryParamsMap queries, const OC::OCRepresentation& rep)
{
    ...
    try {
        if (rep.hasAttribute(m_var_name_temp))
        {
            ...
            rep.getValue(m_var_name_temp, value);
...
OCEntityHandlerResult TempResource::entityHandler(std::shared_ptr<OC::OCResourceRequest> request)
{
    OCEntityHandlerResult ehResult = OC_EH_ERROR;

    if (request)
    {
        std::cout << "In entity handler for TempResource, URI is : "
        ...
        if (requestFlag & RequestHandlerFlag::RequestFlag)
        {
            ...
            {
                std::cout<<"TempResource Get Request"<< std::endl;
